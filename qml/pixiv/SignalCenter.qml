...
QtObject {
    ...
    function selectSection(currentIndex){
        if (!sectionDialogComp){
            sectionDialogComp = Qt.createComponent("Component/SectionDialog.qml");
        }
        var prop = { "prevIndex": currentIndex };
        sectionDialogComp.createObject(pageStack.currentPage, prop);
    }
